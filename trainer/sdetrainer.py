...
class SDETrainer():
    ...
    Trainer class
    """
    def __init__(self, model, cspec_transformer, criterion, metric_ftns, optimizer, config, device, world_size, rank,
                 data_loader, valid_data_loader=None, ema = None,
        ...
        self.n_valid_data_batch = cfg_trainer['n_valid_data_batch']
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (clean, noisy) in enumerate(self.data_loader):
            ...
            loss = self.criterion(torch.view_as_real(output), torch.view_as_real(noise))
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (clean_audio, noisy_audio, _) in enumerate(self.valid_data_loader):
                if batch_idx >= self.n_valid_data_batch:
                    break
                assert clean_audio.shape[0] == 1
                clean_audio, noisy_audio = clean_audio.to(self.device).squeeze(0), noisy_audio.to(self.device).squeeze(0)
                # [1, T]

                norm_factor = noisy_audio.abs().max().item()
                noisy_audio = noisy_audio / norm_factor
                #[1,1,N,L]
                noisy_spec = torch.unsqueeze(self.cspec_transformer.spec_fwd(self.cspec_transformer.stft(noisy_audio)), 0)
                ...
                # infer from noisy noisy_audio input only
                if self.is_distributed:
                    output = self.model.module.infer(noisy_spec)
                else:
                    output = self.model.infer(noisy_spec)

                output = self.cspec_transformer.istft(self.cspec_transformer.spec_back(output.squeeze()), clean_audio.shape[-1])
                ...
                torchaudio.save(self.output_path / f'{batch_idx}.wav', output.cpu(), self.config['sample_rate'])
                torchaudio.save(self.clean_path / f'{batch_idx}.wav', clean_audio.cpu(), self.config['sample_rate'])
                torchaudio.save(self.noisy_path / f'{batch_idx}.wav', noisy_audio.cpu(), self.config['sample_rate'])
